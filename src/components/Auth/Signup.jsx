...
function Signup() {
  ...
  return (
    <div className='w-full min-h-screen px-2 sm:pt-30 pt-15'>
      <h1 className='text-teal-400 text-center text-3xl mb-3'>Sign up</h1>
      <div className="signup max-w-sm mx-auto  bg-gray-900 w-full rounded">
        <form className='w-full p-4 min-h-96  justify-center flex flex-col items-center' onSubmit={handleSubmit(createUserAccount)}>
          ...
          <button className='py-2  mt-5 px-10 rounded bg-teal-500 hover:bg-teal-700 transition-colors delay-100 text-white' type='submit'>{loading ? "Please wait..." : "Create Account"}</button>
          <h1 className='text-white/50 mt-5'>Already have an Account ? <Link to='/login' className='text-teal-400'>Login</Link></h1>
        </form>
      </div>
    </div>
  )
